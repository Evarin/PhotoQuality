...
local function main(params)
   ...
   if qualitynet == nil then
      ...
      local res = computeFeatures(params, style_descrs, {img})
   ...
   local feval = function(x_new)
	  ...
	  targets[i] = iscore
       ...
       local features = computeFeatures(params, style_descrs, inputs)
   ...
   for i=1, #fake_test do
	 ...
	 print('Memory: ', freeMemory/1024/1024, 'MB free of ', totalMemory/1024/1024)
      ...
      if img then
	 local features = computeFeatures(params, style_descrs, {img})
         local prediction = qualitynet:forward(features)
...
function computeFeatures(params, style_descrs, images)
   -- style features
   local imgs = nil
   for i=1, #images do
