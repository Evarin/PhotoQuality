...
local function main(params)
   ...
   local feval = function(x_new)
	  ...
	  img = image.hflip(img)
       ...
       target[1] = math.floor((iscore-1)/10) + 1
       offset = offset + 1
       dl_dx:zero()
       ...
       local m, score 
       m, score = torch.max(prediction)
       score = score[1]*10+5
       local err = math.pow(score - iscore,2)
       toterror = toterror + err

       print('prediction', math.ceil(score), 'objective',  iscore, 'error', err)
   ...
   for i=1, #fake_test do
	 ...
	 local m, score 
         ...
         score = score[1]*10+5
         local err = math.pow(score - iscore,2)
	 print('Note:', math.floor(score+0.5), 'expected:', output[1], 'error:', math.floor(err))
	 toterror = toterror + err
	 fout:write('\n'..nimg..','..math.min(100, math.max(0, score))..','..output[1]) 
      end
   end
   fout:close()
   ...
   for i=1, #test_images do
	 ...
	 local m, score 
         ...
         score = score[1]*10+5
	 print(nimg, score)
	 fout:write('\r'..nimg..';'..math.min(100, math.max(0, math.floor(score)))) 
      end
   end
   fout:close()
end
...
function buildNet(params, res)
   ...
   qualitynet:add(nn.ReLU())
   qualitynet:add(nn.Dropout(params.dropout))
   qualitynet:add(nn.Linear(1024, 1024))
   qualitynet:add(nn.ReLU())
   qualitynet:add(nn.Dropout(params.dropout))
   qualitynet:add(nn.Linear(1024, 1024))
   qualitynet:add(nn.ReLU())
   qualitynet:add(nn.Linear(1024, 10))
   if params.gpu >= 0 then
      if params.backend ~= 'clnn' then
          qualitynet:cuda()
      else
